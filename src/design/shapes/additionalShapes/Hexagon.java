...
public class Hexagon extends Shape implements IPrototype {
	
	private int points = 6, width, height;
	private int[] xPoints = new int[points], yPoints = new int[points],
			outlineXPoints = new int[points], outlineYPoints = new int[points];

	public Hexagon(ShapeConfig shapeConfig) {
		super(shapeConfig);
		setPoints();
	}
	...
	@Override
	public Shape clone() {
		return new Hexagon(this.getShapeConfig());
	}
